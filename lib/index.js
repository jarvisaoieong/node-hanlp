...
module.exports = class HANLP {
  constructor(args) {
    if (!args.properties) {
      return new Error('properties file path is undefined')
    }

    // 初始化配置
    HanLP.initSync(args.properties)
   ...
   */
  Tokenizer(text) {
    let StandardTokenizer = java.import(
      'com.hankcs.hanlp.tokenizer.StandardTokenizer'
    )
   ...
   */
  IndexTokenizer(text) {
    let IndexTokenizer = java.import(
  ...
  ShortSegment(text) {
    let opts = this.options,
      ShortSegment = java.newInstanceSync(
   ...
   */
  NShortSegment(text) {
    let NShortSegment = java.newInstanceSync(
   ...
   */
  SpeedTokenizer(text) {
    let SpeedTokenizer = java.import(
  ...
  Suggester(list, words, Ntop) {
    ...
    words.forEach((item, i) => {
      data[i] = {
        word: item,
        value: JSON.parse(
          Gson.toJsonSync(Suggester.suggestSync(item, Ntop || 1))
        ),
      }
    })
    return data
  ...
  WordDistance(words) {
    words = Array.isArray(words) ? words : new Array(words)
    let data = [],
      SynonymDictionary = java.newInstanceSync(
    ...
    words.forEach((wordA, i) => {
      ...
      words.forEach((wordB, index) => {
        tmp[index] = JSON.parse(
          Gson.toJsonSync(SynonymDictionary.distanceSync(wordA, wordB))
        )
      })
      data[i] = {
        word: wordA,
